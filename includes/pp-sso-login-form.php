...
class PP_SSO_Login_Form {
	...
	/**
	 * Create an instance of the PP_SSO_Login_Form class.
	 *
	 * @param PP_SSO_Option_Settings $settings       A plugin settings object instance.
	 * @param PP_SSO_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 *
	...
	public function handle_login_page( $message ) {
		...
		if ( $this->settings->enable_login ) {
			// SSO is enabled display the login button and messages.

			if ( isset( $_GET['pp-login-error'] ) ) {
				$error_message = ! empty( $_GET['pp-message'] ) ? sanitize_text_field( wp_unslash( $_GET['pp-message'] ) ) : 'Unknown error.';
				$message .= $this->make_error_output( sanitize_text_field( wp_unslash( $_GET['pp-login-error'] ) ), $error_message );
			}
